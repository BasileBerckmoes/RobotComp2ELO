...
using SCB_EZI2C_PDL_v1_0.Controls;

namespace SCB_EZI2C_PDL_v1_0
{
    sealed public class CyCustomizer : ICyParamEditHook_v1, ICyDRCProvider_v1, ICyExprEval_v3
    {
            ...
            ICyExprTypeConverter typeConverter)
        {
            if (args.Length == 1)
            {
                try
                {
                    ...
                    clkMin += clkMin * (CyRanges.CyTolerance.IMO_ACCURACY / 100);
            ...
            ICyExprTypeConverter typeConverter)
        {
            if (args.Length == 0)
            {
                return CyRanges.CyTolerance.ZERO_ACCURACY;
            }
            else
            {
