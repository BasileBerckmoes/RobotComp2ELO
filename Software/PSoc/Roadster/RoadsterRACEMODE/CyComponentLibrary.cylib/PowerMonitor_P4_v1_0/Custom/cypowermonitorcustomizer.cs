...
namespace PowerMonitor_P4_v1_0
{
        ...
        ICyExprEval_v2
    {
        ...
        {
            m_params = new CyParameters(edit);
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            if (m_params == null)
            {
                m_params = new CyParameters(args.InstQueryV1);
            }
        ...
        public CyExprEvalFuncEx GetFunction(string exprFuncName)
        {
            switch (exprFuncName)
            {
                case "GetAdcClockDivider":
                    return new CyExprEvalFuncEx(GetAdcClockDivider);
                case "GetAdcClock":
                    return new CyExprEvalFuncEx(GetAdcClock);
        ...
        object GetAdcClockDivider(string exprFuncName, object[] callingArgs, ICyExprEvalArgs_v2 creatorArgs,
            ICyExprTypeConverter typeConverter)
        {
            ...
            if (creatorArgs.InstQuery != null)
            {
                parameters = new CyParameters(creatorArgs.InstQuery);
            }

            int result = CyClock.GetAdcClockDivider(parameters);

            return result;
        }

        object GetAdcClock(string exprFuncName, object[] callingArgs, ICyExprEvalArgs_v2 creatorArgs,
            ICyExprTypeConverter typeConverter)
        {
            if (callingArgs.Length != 1)
            {
                return new CyCustErr(string.Format(
                    "Incorrect number of arguments to call function '{0}'. Expected 1.",
                    exprFuncName));
            }
            string adClkString = (string)callingArgs[0];
            ...
            CyClock.AdcClkInfo AdcClockConfig = CyClock.OptimizeClockApertures(parameters);
            switch (adClkString.ToUpper())
            {
                case "A":
                    return AdcClockConfig.AdcClockApertures[0];
                case "B":
                    return AdcClockConfig.AdcClockApertures[1];
                case "C":
                    return AdcClockConfig.AdcClockApertures[2];
                case "D":
                    return AdcClockConfig.AdcClockApertures[3];
                case "SCANTIME":
                    return AdcClockConfig.ScanTime;
                case "RSPTIME":
                    return AdcClockConfig.ResponseTime;
                case "ADCCLKSELECT":
                    string apertureSelect = "{ ";
                    for (int i = 0; i < AdcClockConfig.ConverterAdcClockSelect.Length; i++)
                    {
                        apertureSelect += (i < AdcClockConfig.ConverterAdcClockSelect.Length - 1) ?
                            AdcClockConfig.ConverterAdcClockSelect[i].ToString() + "u, " :
                            AdcClockConfig.ConverterAdcClockSelect[i].ToString() + "u }";
                    }
                    //return "{0,1,2,3,0,1,2,3}";
                    return apertureSelect;
                default:
                    return new CyCustErr(string.Format(
                    "Invalid argument to function '{0}'. Must be A, B, C, D, ScanTime, RspTime.", exprFuncName));
            }
        }
