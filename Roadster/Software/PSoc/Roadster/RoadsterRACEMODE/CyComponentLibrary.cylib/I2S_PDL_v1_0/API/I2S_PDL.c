...
*
* \brief
*  This file contains setup, control and status commands for the I2S
*  component.
*
********************************************************************************
* \copyright
...
`=$CY_CONST_CONFIG ? "const " : ""`cy_stc_i2s_config_t `$INSTANCE_NAME`_config =
{
    ...
    .txOverheadValue    = `=$TxOverheadValue ? true : false`,
    ...
    .rxBitExtension     = `=$RxBitExtension ? true : false`,
...
/* An internal structure to save the current TX/RX enable/disable Component
*  status and current interrupt settings (the Component settings that can 
*  be changed on the fly by the component API) during stop/start sequence.
*/
...
void `$INSTANCE_NAME`_Start(void)
{
    ...
    if(0U == `$INSTANCE_NAME`_initVar)
    {
        ...
        `$INSTANCE_NAME`_backup.enableState = 0UL;
        `$INSTANCE_NAME`_backup.interruptMask = `$INSTANCE_NAME`_INT_MASK;
        
    #if(`$INSTANCE_NAME`_TX_ENABLED)
    	`$INSTANCE_NAME`_backup.enableState |= I2S_CMD_TX_START_Msk;
    #endif /* `$INSTANCE_NAME`_TX_ENABLED */

    #if(`$INSTANCE_NAME`_RX_ENABLED)
    	`$INSTANCE_NAME`_backup.enableState |= I2S_CMD_RX_START_Msk;
    ...
    if(0UL != (`$INSTANCE_NAME`_backup.enableState & I2S_CMD_TX_START_Msk))
    {
        ...
        /* The first transmission data setting. Write at least one frame to FIFO */
        for(locChanCnt = 0UL; locChanCnt < `$INSTANCE_NAME`_CHAN_NMBR; locChanCnt++)
        {
...
* the PDL API) and then disables interrupts and TX and/or RX operation.
*
*******************************************************************************/
