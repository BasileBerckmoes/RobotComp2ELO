...
cy_stc_scb_spi_config_t `=$CY_CONST_CONFIG ? "const " : ""``$INSTANCE_NAME`_config =
{
    ...
    .ssPolarity               = ((`=                 (0 != $Ss0Polarity)  ? "CY_SCB_SPI_ACTIVE_HIGH" : "CY_SCB_SPI_ACTIVE_LOW"` << `$INSTANCE_NAME`_SPI_SLAVE_SELECT0) | \
                                 (`= ($MasterMode && (0 != $Ss1Polarity)) ? "CY_SCB_SPI_ACTIVE_HIGH" : "CY_SCB_SPI_ACTIVE_LOW"` << `$INSTANCE_NAME`_SPI_SLAVE_SELECT1) | \
                                 (`= ($MasterMode && (0 != $Ss2Polarity)) ? "CY_SCB_SPI_ACTIVE_HIGH" : "CY_SCB_SPI_ACTIVE_LOW"` << `$INSTANCE_NAME`_SPI_SLAVE_SELECT2) | \
                                 (`= ($MasterMode && (0 != $Ss3Polarity)) ? "CY_SCB_SPI_ACTIVE_HIGH" : "CY_SCB_SPI_ACTIVE_LOW"` << `$INSTANCE_NAME`_SPI_SLAVE_SELECT3)),
