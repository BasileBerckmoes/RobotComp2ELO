...
/* Register callback. */
__STATIC_INLINE void `$INSTANCE_NAME`_RegisterCallback(scb_spi_handle_events_t callback);

/* Bus state. */
__STATIC_INLINE bool `$INSTANCE_NAME`_IsBusBusy(void);

/* Slave select control. */
__STATIC_INLINE void `$INSTANCE_NAME`_SetActiveSlaveSelect(uint32_t slaveSelect);
__STATIC_INLINE void `$INSTANCE_NAME`_SetActiveSlaveSelectPolarity(uint32_t slaveSelect, uint32_t polarity);

/* Low level: read. */
__STATIC_INLINE uint32_t `$INSTANCE_NAME`_Read(void);
__STATIC_INLINE uint32_t `$INSTANCE_NAME`_ReadArray(void *rxBuf, uint32_t size);
...
__STATIC_INLINE uint32_t `$INSTANCE_NAME`_WriteArray(void *txBuf, uint32_t size);
__STATIC_INLINE void     `$INSTANCE_NAME`_WriteArrayBlocking(void *txBuf, uint32_t size);
...
__STATIC_INLINE cy_en_scb_spi_status_t `$INSTANCE_NAME`_Transfer(void *txBuf, void *rxBuf, uint32_t size);
...
#define `$INSTANCE_NAME`_SPI_SLAVE_SELECT0    (`$INSTANCE_NAME`_SCB__SS0_POSITION)

/** The slave select line 1 constant which takes into account pin placement */
#define `$INSTANCE_NAME`_SPI_SLAVE_SELECT1    (`$INSTANCE_NAME`_SCB__SS1_POSITION)

/** The slave select line 2 constant which takes into account pin placement */
#define `$INSTANCE_NAME`_SPI_SLAVE_SELECT2    (`$INSTANCE_NAME`_SCB__SS2_POSITION)

/** The slave select line 3 constant which takes into account pin placement */
#define `$INSTANCE_NAME`_SPI_SLAVE_SELECT3    (`$INSTANCE_NAME`_SCB__SS3_POSITION)
/** @} group_macros */
...
__STATIC_INLINE uint32_t `$INSTANCE_NAME`_ReadArray(void *rxBuf, uint32_t size)
{
    return Cy_SCB_SPI_ReadArray(`$INSTANCE_NAME`_HW, rxBuf, size);
}
...
__STATIC_INLINE uint32_t `$INSTANCE_NAME`_WriteArray(void *txBuf, uint32_t size)
{
    return Cy_SCB_SPI_WriteArray(`$INSTANCE_NAME`_HW, txBuf, size);
}
...
__STATIC_INLINE void `$INSTANCE_NAME`_WriteArrayBlocking(void *txBuf, uint32_t size)
{
    Cy_SCB_SPI_WriteArrayBlocking(`$INSTANCE_NAME`_HW, txBuf, size);
}
...
__STATIC_INLINE cy_en_scb_spi_status_t `$INSTANCE_NAME`_Transfer(void *txBuf, void *rxBuf, uint32_t size)
{
    return Cy_SCB_SPI_Transfer(`$INSTANCE_NAME`_HW, txBuf, rxBuf, size, &`$INSTANCE_NAME`_context);
}
