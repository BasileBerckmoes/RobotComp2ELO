...
__STATIC_INLINE void     `$INSTANCE_NAME`_CacheInvalidate(uint32_t cacheType);
...
#define `$INSTANCE_NAME`_RX_CLOCK_SELECT         (1u)            /**< Clock source for the receiver clock */
...
__STATIC_INLINE void `$INSTANCE_NAME`_CacheInvalidate(uint32_t cacheType)
{
    Cy_SMIF_CacheInvalidate(`$INSTANCE_NAME`_HW , cacheType);
}
