...
cy_stc_smif_config_t `=$CY_CONST_CONFIG ? "const" : ""` `$INSTANCE_NAME`_config =
{
    ...
    .rxClockSel     = `$INSTANCE_NAME`_RX_CLOCK_SELECT,
...
cy_en_smif_status_t `$INSTANCE_NAME`_Start(cy_stc_smif_block_config_t *blockConfig, uint32_t timeoutMs)
{
    if (0U == `$INSTANCE_NAME`_initVar)
    {
        ...
        Cy_SMIF_CacheEnable(`$INSTANCE_NAME`_HW, CY_SMIF_CACHE_BOTH);
        Cy_SMIF_CachePrefetchingEnable(`$INSTANCE_NAME`_HW, CY_SMIF_CACHE_BOTH);
