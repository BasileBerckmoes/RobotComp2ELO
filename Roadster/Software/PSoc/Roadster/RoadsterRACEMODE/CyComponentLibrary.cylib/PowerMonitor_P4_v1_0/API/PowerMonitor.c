...
void `$INSTANCE_NAME`_Init(void)
{
    ...
    /* Enable DMA */
    `$INSTANCE_NAME`_DMA_Enable_Start((void *) &`$INSTANCE_NAME`_faultConfig, (void *) `$INSTANCE_NAME`_HwComp_MyEnable__CONTROL_REG);
    ...
    /* Check DMA */
    `$INSTANCE_NAME`_DMA_Check_Start((void *) `$INSTANCE_NAME`_HwComp_MyEnable__CONTROL_REG, (void *) `$INSTANCE_NAME`_HwComp_Comp_u0__A1_REG);
...
void `$INSTANCE_NAME`_EnableFault(uint32 converterMask) 
{
    ...
    for (converter = 0u; converter < `$INSTANCE_NAME`_NUM_CONVERTERS; converter++)
    {
        if (0u != (converterMask & (1uL << converter)))
        {
            ...
            `$INSTANCE_NAME`_faultConfig[converter] = (converterNum | enableShifted);
        }
    }
}
...
uint32 `$INSTANCE_NAME`_GetFaultSource(void) 
{
        ...
        {    
            ...
            for (chan = 0u; chan < `$INSTANCE_NAME`_NUM_CONVERTERS; chan++)
            {
                ...
                if (`$INSTANCE_NAME`_faultConfig[chan] & `$INSTANCE_NAME`_ENABLE_UV_MASK)
                {
                    if (chanCounts <= `$INSTANCE_NAME`_UVFaultThreshold_Count[chan])  
                    {
                        `$INSTANCE_NAME`_UVFaultStatus |= (1uL << chan);
                        `$INSTANCE_NAME`_faultConfig[chan] &= ~`$INSTANCE_NAME`_ENABLE_UV_MASK;
                    }
                }
                    
                /* Update the OVFaultStatus if active.*/
                if (`$INSTANCE_NAME`_faultConfig[chan] & `$INSTANCE_NAME`_ENABLE_OV_MASK)
                {
                    if (chanCounts >= `$INSTANCE_NAME`_OVFaultThreshold_Count[chan])  
                    {
                        ...
                        `$INSTANCE_NAME`_faultConfig[chan] &= 
                             ~(`$INSTANCE_NAME`_ENABLE_OV_MASK | `$INSTANCE_NAME`_ENABLE_UV_MASK);
                    }
                }
            }
        }
