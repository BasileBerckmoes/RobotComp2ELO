/*******************************************************************************
* File Name: JoyStickADC_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_Start
********************************************************************************
*
* Summary:
...
void JoyStickADC_IntClock_Start(void) 
{
    ...
    JoyStickADC_IntClock_CLKEN |= JoyStickADC_IntClock_CLKEN_MASK;
	JoyStickADC_IntClock_CLKSTBY |= JoyStickADC_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: JoyStickADC_IntClock_Stop
********************************************************************************
*
* Summary:
...
void JoyStickADC_IntClock_Stop(void) 
{
    ...
    JoyStickADC_IntClock_CLKEN &= (uint8)(~JoyStickADC_IntClock_CLKEN_MASK);
	JoyStickADC_IntClock_CLKSTBY &= (uint8)(~JoyStickADC_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((JoyStickADC_IntClock_CLKEN & JoyStickADC_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = JoyStickADC_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = JoyStickADC_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* JoyStickADC_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(JoyStickADC_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(JoyStickADC_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = JoyStickADC_IntClock_GetSourceRegister();
    uint16 oldDivider = JoyStickADC_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = JoyStickADC_IntClock_CLKEN & JoyStickADC_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(JoyStickADC_IntClock_DIV_PTR, clkDivider);
                JoyStickADC_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                JoyStickADC_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(JoyStickADC_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((JoyStickADC_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(JoyStickADC_IntClock_DIV_PTR, clkDivider);
				JoyStickADC_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(JoyStickADC_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: JoyStickADC_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    JoyStickADC_IntClock_MOD_SRC |= modeBitMask & (uint8)JoyStickADC_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: JoyStickADC_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    JoyStickADC_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(JoyStickADC_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: JoyStickADC_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return JoyStickADC_IntClock_MOD_SRC & (uint8)(JoyStickADC_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: JoyStickADC_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = JoyStickADC_IntClock_GetDividerRegister();
    uint8 oldSrc = JoyStickADC_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        JoyStickADC_IntClock_MOD_SRC |= CYCLK_SSS;
        JoyStickADC_IntClock_MOD_SRC =
            (JoyStickADC_IntClock_MOD_SRC & (uint8)(~JoyStickADC_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return JoyStickADC_IntClock_MOD_SRC & JoyStickADC_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: JoyStickADC_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    JoyStickADC_IntClock_PHASE = clkPhase & JoyStickADC_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: JoyStickADC_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return JoyStickADC_IntClock_PHASE & JoyStickADC_IntClock_PHASE_MASK;
}
