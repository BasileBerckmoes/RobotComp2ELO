...
void LCD1_LoadCustomFonts(uint8 const customData[]) 
{
    ...
    */
    LCD1_WriteControl(LCD1_CGRAM_0);

    /* Load in the 64 bytes of CustomChar Data */
    for(indexU8 = 0u; indexU8 < LCD1_CUSTOM_CHAR_SET_LEN; indexU8++)
    {
        LCD1_WriteData(customData[indexU8]);
    }

    LCD1_IsReady();
    LCD1_WriteControl(LCD1_DDRAM_0);
}
