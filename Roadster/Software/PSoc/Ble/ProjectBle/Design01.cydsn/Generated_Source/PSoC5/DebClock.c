...
void DebClock_Start(void) 
{
    ...
    DebClock_CLKEN |= DebClock_CLKEN_MASK;
	DebClock_CLKSTBY |= DebClock_CLKSTBY_MASK;
}
...
void DebClock_Stop(void) 
{
    ...
    DebClock_CLKEN &= (uint8)(~DebClock_CLKEN_MASK);
	DebClock_CLKSTBY &= (uint8)(~DebClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DebClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((DebClock_CLKEN & DebClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = DebClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DebClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DebClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DebClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DebClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DebClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DebClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = DebClock_GetSourceRegister();
    uint16 oldDivider = DebClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DebClock_CLKEN & DebClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DebClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(DebClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: DebClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    DebClock_MOD_SRC |= modeBitMask & (uint8)DebClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: DebClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    DebClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DebClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: DebClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return DebClock_MOD_SRC & (uint8)(DebClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: DebClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = DebClock_GetDividerRegister();
    uint8 oldSrc = DebClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DebClock_MOD_SRC |= CYCLK_SSS;
        DebClock_MOD_SRC =
            (DebClock_MOD_SRC & (uint8)(~DebClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DebClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return DebClock_MOD_SRC & DebClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DebClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    DebClock_PHASE = clkPhase & DebClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DebClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return DebClock_PHASE & DebClock_PHASE_MASK;
}
